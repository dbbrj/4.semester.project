    ...
    private int Find_EmptyTray_inInventory()
    {
        ...
        try
        {
            ...
            while (keys.hasNext())
            {
                ...
                String content = this.warehouse_Inventory.get(key).toString();
    ...
    private boolean Update_WarehouseInventory(String warehouseInventory_string)
    {
        ...
        try
        {
            ...
            for (int i = 0; i < inventoryList.length(); i++)
            {
                ...
                // Each entry has named fields "Id" (int) and "Content" (string)
                int    trayId  = entry.getInt("Id");
                String content = entry.optString("Content", "");
                this.warehouse_Inventory.put(String.valueOf(trayId), content);
            }
